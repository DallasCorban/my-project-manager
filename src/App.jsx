...
const Sidebar = ({
  ...
  setDarkMode,
    ...
    <div className={`w-64 border-r flex flex-col hidden md:flex ${darkMode ? 'bg-[#111322] border-[#323652]' : 'bg-white border-[#bec3d4]'}`}>
        <div className={`p-4 border-b ${darkMode ? 'border-[#323652]' : 'border-[#bec3d4]'} `}><h2 className="font-bold text-lg">Workspaces</h2></div>
        <div className="flex-1 overflow-y-auto p-3 space-y-6">
           <div>
             ...
             {workspaces.length > 0 ? (
               ...
               <div className={`text-xs px-3 py-2 rounded-md border ${darkMode ? "border-[#323652] text-gray-500" : "border-gray-300 text-gray-500"}`}>
        ...
        <div className={`p-4 border-t ${darkMode ? 'border-[#323652]' : 'border-[#bec3d4]'} flex items-center justify-between`}>
             <span className="text-xs text-gray-500">Theme</span>
             <button onClick={() => setDarkMode(!darkMode)} className={`p-2 rounded-full ${darkMode ? 'bg-[#323652] text-yellow-400' : 'bg-gray-100 text-gray-600'}`}>{darkMode ? <Sun size={16} /> : <Moon size={16} />}</button>
        </div>
    </div>
);
...
const AppHeader = ({ activeEntity, activeTab, setActiveTab, darkMode, setSettingsMenuOpen, settingsMenuOpen, showWeekends, onToggleWeekends, showLabels, setShowLabels, colorBy, setColorBy, zoomLevel, handleZoomChange, rowHeight, setRowHeight, isChatOpen, setIsChatOpen, scrollToToday, updateEntityName, canEditEntityName = true, onExport, onExportJson, onImportJson, onResetData, authUser, onOpenAuth, onOpenMembers }) => (
    <>
        <div className={`h-16 border-b px-8 flex items-center justify-between shrink-0 ${darkMode ? 'border-[#323652] bg-[#181b34]' : 'border-[#bec3d4] bg-white'}`}>
          <div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition-colors ${
                darkMode ? 'bg-[#1c213e] border-[#323652] text-gray-200 hover:bg-[#202336]' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'
              }`}
              disabled={!authUser || authUser.isAnonymous}
            >
              Members
            </button>
            <button onClick={onOpenAuth} className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition-colors ${darkMode ? 'bg-[#1c213e] border-[#323652] text-gray-200 hover:bg-[#202336]' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
              {authUser ? (authUser.isAnonymous ? 'Guest' : (authUser.email || 'Account')) : 'Account'}
            </button>
          </div>
        </div>
        <div className={`px-8 border-b flex items-center justify-between shrink-0 sticky top-0 z-[80] ${darkMode ? 'border-[#323652] bg-[#181b34]' : 'border-[#bec3d4] bg-white'}`}>
          ...
          <div className="flex items-center gap-6">
            {activeTab === 'gantt' && (
              <div className="flex gap-4">
                  <button onClick={() => scrollToToday(true)} className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${darkMode ? 'bg-[#1c213e] border-[#323652] text-gray-300 hover:bg-[#202336]' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-100'}`}><Target size={14} /> Today</button>
                  <div className="relative">
                     <button onClick={(e) => { e.stopPropagation(); setSettingsMenuOpen(!settingsMenuOpen); }} className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${settingsMenuOpen || !darkMode ? 'bg-white text-gray-700 border-gray-300' : 'bg-[#1c213e] border-[#323652] text-gray-300'}`}><Settings size={14} /> Settings</button>
                     {settingsMenuOpen && (
                         <div className={`absolute top-full right-0 mt-2 w-56 rounded-lg shadow-xl border z-[130] p-2 animate-in fade-in zoom-in-95 duration-100 ${darkMode ? 'bg-[#323652] border-[#3d4058]' : 'bg-white border-gray-300'}`} onClick={e => e.stopPropagation()}>
                 ...
                 <div className={`flex items-center gap-3 px-3 py-1.5 rounded-full border ${darkMode ? 'bg-[#1c213e] border-[#323652] text-gray-300' : 'bg-gray-50 border-gray-300 text-gray-600'}`}><ZoomOut size={14} /><input type="range" min="10" max="100" value={zoomLevel} onChange={handleZoomChange} className="w-20 h-1 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#0073ea]" /><ZoomIn size={14} /></div>
                 <div className={`flex items-center gap-3 px-3 py-1.5 rounded-full border ${darkMode ? 'bg-[#1c213e] border-[#323652] text-gray-300' : 'bg-gray-50 border-gray-300 text-gray-600'}`}><ArrowUpDown size={14} /><input type="range" min="32" max="80" value={rowHeight} onChange={(e) => setRowHeight(Number(e.target.value))} className="w-20 h-1 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#0073ea]" /></div>
              </div>
            )}
...
const GroupHeaderRow = ({ darkMode, boardColumns, onStartResize }) => {
    ...
    return (
        <div className={`flex border-b text-xs font-bold text-gray-500 uppercase tracking-wide ${darkMode ? 'bg-[#181b34] border-[#323652]' : 'bg-white border-[#bec3d4]'}`}>
            <div
                className={`border-r flex items-center justify-center py-2 relative min-w-0 ${darkMode ? 'border-[#323652]' : 'border-[#bec3d4]'}`}
            ...
            <div
                className={`border-r px-4 py-2 flex items-center relative min-w-0 ${darkMode ? 'border-[#323652]' : 'border-[#bec3d4]'}`}
            ...
            <div
                className={`border-r px-4 py-2 flex items-center justify-center relative min-w-0 ${darkMode ? 'border-[#323652]' : 'border-[#bec3d4]'}`}
...
const LabelEditorModal = ({ isOpen, onClose, items, onSave, title, darkMode }) => {
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className={`w-96 rounded-lg shadow-2xl p-6 ${darkMode ? 'bg-[#323652] text-white' : 'bg-white text-gray-800'}`} onClick={e => e.stopPropagation()}>
...
const StatusDropdown = ({ statuses, currentStatusId, onSelect, darkMode, onEdit, onAddLabel }) => {
    ...
    return (
        <div className={`w-64 rounded-2xl shadow-2xl border overflow-hidden animate-in fade-in zoom-in-95 duration-100 ${darkMode ? 'bg-[#161a33] border-[#323652]' : 'bg-white border-gray-300'}`}>
            ...
            <div className={`px-3 py-3 border-t ${darkMode ? 'border-[#323652]' : 'border-gray-100'}`}>
            ...
            <button
                ...
                className={`w-full px-4 py-2.5 text-[11px] font-semibold border-t flex items-center gap-2 transition-colors ${
                    darkMode
                        ? 'border-[#323652] text-blue-300 hover:bg-[#0f1224]'
...
const UpdatesPanel = ({
    ...
    return (
        <div className={`fixed top-0 right-0 bottom-0 w-[420px] z-[160] border-l shadow-2xl flex flex-col ${darkMode ? 'bg-[#151726] border-[#323652]' : 'bg-white border-gray-300'}`}>
            <div className={`px-5 py-4 border-b ${darkMode ? 'border-[#323652]' : 'border-gray-300'}`}>
                ...
                {activeTab === "updates" && (
                    <>
                        <div className="p-4 border-b border-transparent">
                            <div className={`rounded-xl border ${darkMode ? 'border-[#323652] bg-[#0f1224]' : 'border-gray-300 bg-gray-50'}`}>
                                    ...
                                    <>
                                        <div className="px-3 pt-2">
                                            <input
                                                ...
                                                className={`w-full h-8 px-2.5 rounded-md text-xs outline-none border ${
                                                    darkMode
                                                        ? 'bg-[#0b0e1c] border-[#323652] text-gray-200 placeholder-gray-500'
                        ...
                        <div className="px-4 pb-6">
                            ...
                            {updates.map((u) => (
                                <div key={u.id} className={`mt-4 rounded-xl border ${darkMode ? 'border-[#323652] bg-[#101328]' : 'border-gray-300 bg-white'} p-3`}>
                                    ...
                                    {openReplyId === u.id && (
                                        <div className={`mt-3 rounded-lg border ${darkMode ? 'border-[#323652] bg-[#0f1224]' : 'border-gray-300 bg-gray-50'} p-2`}>
                                            <textarea
                    ...
                    >
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-xs font-semibold border transition-colors ${
                                        canUploadFiles
                                            ? darkMode
                                                ? 'border-[#323652] text-gray-200 hover:bg-white/10'
                                                : 'border-gray-300 text-gray-700 hover:bg-gray-100'
                                            : darkMode
                                            ? 'border-[#323652] text-gray-500'
                                    ...
                                    <button
                                        ...
                                        className={`p-2 rounded-md border ${
                                            fileView === "grid"
                                                ...
                                                : darkMode ? 'border-[#323652] text-gray-400 hover:bg-white/5' : 'border-gray-300 text-gray-500 hover:bg-gray-100'
                                        }`}
                                    >
                                        <LayoutGrid size={14} />
                                    </button>
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-8">
                            {!canViewFiles ? (
                                <div className={`h-full rounded-2xl border flex flex-col items-center justify-center text-center gap-3 ${
                                    darkMode ? 'border-[#323652] bg-[#0f1224]' : 'border-gray-300 bg-gray-50'
                                }`}>
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`rounded-xl border overflow-hidden ${
                                                    darkMode ? 'border-[#323652] bg-[#0f1224]' : 'border-gray-300 bg-white'
                                                                ...
                                                                <div className="mt-2">
                                                                    <select
                                                                        ...
                                                                        className={`text-[11px] px-2 py-1 rounded border outline-none ${
                                                                            darkMode
                                                                                ? 'bg-[#0b0e1c] border-[#323652] text-gray-200'
                                                                                : 'bg-white border-gray-300 text-gray-700'
                                                                        }`}
                                                                    >
                                                                ...
                                                                <div className="mt-2 flex items-center gap-2 text-[11px]">
                                                                    <button
                                                                        ...
                                                                        className={`px-2 py-1 rounded border ${
                                                                            file.access?.allowShareLink
                                                                                ? darkMode ? 'border-blue-500/40 text-blue-200 bg-blue-500/10' : 'border-blue-500/30 text-blue-700 bg-blue-50'
                                                                                : darkMode ? 'border-[#323652] text-gray-400' : 'border-gray-300 text-gray-500'
                                                                        }`}
                                                                    >
                                                                    ...
                                                                    {file.access?.allowShareLink && file.access?.shareToken && shareBaseUrl && (
                                                                        <button
                                                                            ...
                                                                            className={`px-2 py-1 rounded border ${
                                                                                darkMode ? 'border-[#323652] text-gray-200' : 'border-gray-300 text-gray-600'
                                                                            }`}
                                                                        >
                                                                            Copy link
                                                                        </button>
                        ...
                        {previewFile && (
                            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60" onClick={() => setPreviewFile(null)}>
                                ...
                                >
                                    <div className={`px-4 py-3 border-b flex items-center justify-between ${darkMode ? "border-[#323652]" : "border-gray-300"}`}>
        ...
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div
                className={`w-[640px] max-w-[95vw] max-h-[85vh] rounded-xl shadow-2xl p-6 overflow-y-auto ${
                    darkMode ? "bg-[#1c213e] text-white border border-[#323652]" : "bg-white text-gray-900 border border-gray-300"
                }`}
                onClick={(e) => e.stopPropagation()}
            >
                ...
                {canManage ? (
                    <div className={`p-4 rounded-lg border mb-5 ${darkMode ? "border-[#323652] bg-[#151726]" : "border-gray-300 bg-gray-50"}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                            <input
                                ...
                                className={`md:col-span-2 h-9 px-3 rounded-md text-xs outline-none border ${
                                    darkMode ? "bg-[#0f1224] border-[#323652] text-gray-200" : "bg-white border-gray-300 text-gray-700"
                                }`}
                            />
                            <select
                                value={inviteRole}
                ...
                <div className="space-y-3">
                    ...
                    {members.map((member) => {
                        ...
                        return (
                            <div key={memberId} className={`p-3 rounded-lg border ${darkMode ? "border-[#323652] bg-[#151726]" : "border-gray-300 bg-gray-50"}`}>
                ...
                {canManage && invites.length > 0 && (
                    <div className="mt-6 space-y-3">
                        ...
                        {invites.filter((i) => i.status === "pending").map((invite) => (
                            <div key={invite.id} className={`p-3 rounded-lg border flex items-center justify-between ${darkMode ? "border-[#323652] bg-[#151726]" : "border-gray-300 bg-gray-50"}`}>
                                <div>
...
const DatePicker = ({ datePickerOpen, setDatePickerOpen, darkMode, updateTaskDate }) => {
    ...
    return (
        <>
          ...
          <div
            className={`fixed z-[120] w-64 rounded-lg shadow-2xl border p-3 flex flex-col gap-2 ${darkMode ? 'bg-[#323652] border-[#3d4058] text-white' : 'bg-white border-gray-300 text-gray-800'}`}
            style={{ top, left }}
                         ...
                         {defaultGroups.map(group => {
                             ...
                             return (
                                 <div key={group.id} className="mb-8">
                                     ...
                                     >
                                         <div onClick={() => toggleGroupCollapse(group.id)} className={`p-1 rounded cursor-pointer transition ${isGroupCollapsed ? '-rotate-90' : 'rotate-0'} ${darkMode ? 'hover:bg-[#323652]' : 'hover:bg-gray-100'}`}><ChevronDown size={18} style={{ color: group.color }} /></div>
                                     ...
                                     {!isGroupCollapsed ? (
                                         <div className={`shadow-sm border-l-4 rounded-tl-md rounded-bl-md overflow-visible ${darkMode ? 'border-[#323652]' : 'border-gray-300'}`} style={{ borderLeftColor: group.color }}>
                                             ...
                                             <div className={`flex h-10 items-center border-b ${darkMode ? 'border-[#323652] hover:bg-[#202336] bg-[#181b34]' : 'border-[#eceff8] hover:bg-[#f5f6f8] bg-white'}`}>
                                                 <div className={`border-r h-full ${darkMode ? 'border-[#323652]' : 'border-[#eceff8]'}`} style={{ width: boardColumns.select }}></div>
                                          ...
                                          <div className="h-10 flex items-center rounded-md overflow-hidden relative pl-4" style={{ backgroundColor: darkMode ? '#323652' : '#f0f0f0' }}><div className="absolute left-0 top-0 bottom-0 w-1.5" style={{ backgroundColor: group.color }}></div><div className="flex-1 flex h-full">{statuses.map(s => { const count = groupTasks.filter(t => t.status === s.id).length; if (count === 0) return null; const pct = (count / groupTasks.length) * 100; return <div key={s.id} style={{ width: `${pct}%`, backgroundColor: s.color }} title={`${s.label}: ${count}`} className="h-full first:rounded-l-none" />; })}</div><div className={`w-32 flex items-center justify-center text-xs font-bold ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{groupTasks.length} Items</div></div>
                                       )}
                                  </div>
                             );
                         })}
                         <button onClick={() => canEdit && addGroup(proj.id)} className={`flex items-center gap-2 text-sm font-medium px-4 py-2 rounded border transition-colors ${darkMode ? 'border-[#323652] hover:bg-[#323652] text-gray-300' : 'border-gray-300 hover:bg-gray-50 text-gray-600'} ${!canEdit ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!canEdit}><Plus size={16} /> Add New Group</button>
                    </div>
                );
            })}
        </div>
        ...
        <div className={`flex-1 flex flex-col min-w-0 relative ${darkMode ? 'bg-[#181b34]' : 'bg-white'}`}>
            ...
            <div className={`flex-1 overflow-hidden flex flex-col relative ${darkMode ? 'bg-[#181b34]' : 'bg-white'}`}>
                {activeEntity?.type === 'workspace' && visibleProjects.length === 0 ? (
                    <div className="flex-1 flex items-center justify-center px-6">
                        <div className={`w-[620px] max-w-full rounded-2xl border p-8 text-center ${
                            darkMode ? 'bg-[#151726] border-[#323652]' : 'bg-white border-gray-300'
                        }`}>
                ...
                {authUser && !authUser.isAnonymous && activeProject && !activePermissions.canView && (
                    <div className="absolute inset-0 z-[140] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                        <div className={`w-[520px] max-w-[90vw] rounded-xl p-6 shadow-2xl ${darkMode ? 'bg-[#151726] border border-[#323652]' : 'bg-white border border-gray-300'}`}>
                ...
                {authUser && !authUser.isAnonymous && activeProject && activePermissions.canView && !canEditActiveProject && (
                    <div className="absolute top-4 right-4 z-[135]">
                        <div className={`px-3 py-1.5 rounded-md text-xs font-semibold border ${
                            darkMode ? 'bg-[#0f1224] border-[#323652] text-amber-300' : 'bg-amber-50 border-amber-200 text-amber-700'
                        }`}>
            ...
            {showDebugBadge && (
                <div className={`fixed bottom-4 right-4 z-[260] rounded-lg text-[10px] font-mono shadow-lg ${
                    darkMode ? "bg-[#0f1224] border border-[#323652] text-gray-200" : "bg-white border border-gray-300 text-gray-700"
                }`}>
                    <button
