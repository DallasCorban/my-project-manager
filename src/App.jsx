...
function useHybridState(key, initialValue, collectionName) {
  ...
  // 2) Subscribe to Firestore doc updates
  useEffect(() => {
    if (!user || !db) return;

    const appId = "my-manager-app";
    const docRef = doc(db, "artifacts", appId, collectionName, key);
...
const GanttView = (props) => {
  const {
    ...
    return (
        <div ref={props.bodyRef} className={`flex-1 overflow-auto overflow-x-auto relative border-t ${darkMode ? 'bg-[#181b34] border-[#2b2c32]' : 'bg-white border-[#d0d4e4]'}`}>
             <div className={`flex border-b sticky top-0 z-40 shadow-sm ${darkMode ? 'bg-[#181b34] border-[#2b2c32]' : 'bg-white border-[#d0d4e4]'}`} style={{ width: `calc(20rem + ${visibleDays.length * zoomLevel}px)` }}>
                <div className={`w-80 border-r p-3 font-medium text-sm pl-6 sticky left-0 z-50 flex-shrink-0 ${darkMode ? 'bg-[#1c213e] border-[#2b2c32] text-gray-300' : 'bg-[#f9fafc] border-[#d0d4e4] text-gray-500'}`}>Project / Task</div>
                <div className="flex flex-col flex-1">
                    ...
                    <div className="flex h-10">
                        {visibleDays.map((day, i) => {
                            ...
                            return (
                                <div key={i} className={`flex-shrink-0 border-r flex flex-col items-center justify-center text-gray-400 relative ${darkMode ? 'border-white/5 ' + (day.isWeekend?'bg-[#151726]':'bg-[#181b34]') : 'border-[#eceff8] ' + (day.isWeekend?'bg-slate-50':'bg-white')} ${!showWeekends && i > 0 && day.index > visibleDays[i-1].index + 1 ? (darkMode ? 'border-l-2 border-l-[#3e3f4b]' : 'border-l-2 border-l-gray-300') : ''} ${day.isToday ? 'bg-blue-600 text-white' : ''}`} style={{ width: `${zoomLevel}px` }}>
             ...
             <div style={{ width: `calc(20rem + ${visibleDays.length * zoomLevel}px)` }}>
                {visibleProjects.map((project) => {
                     ...
                     return (
                        <div key={project.id} className={`border-b group/project ${darkMode ? 'border-[#2b2c32] bg-[#181b34]' : 'border-[#eceff8] bg-white'}`}>
                                ...
                                return (
                                    <div key={group.id}>
                                        <div className="flex relative transition-colors duration-200" style={{ height: `${rowHeight}px`, backgroundColor: group.color + (darkMode ? '33' : '1A') }}>
                                            <div className={`w-80 border-r px-4 flex items-center gap-2 sticky left-0 z-40 cursor-pointer flex-shrink-0 transition-colors ${darkMode ? 'border-[#2b2c32] text-gray-200' : 'border-[#d0d4e4] text-gray-700'}`} onClick={() => toggleGroupCollapse(group.id)} style={{ background: `linear-gradient(${group.color}${darkMode ? '33' : '1A'}, ${group.color}${darkMode ? '33' : '1A'}), ${darkMode ? '#181b34' : '#ffffff'}` }}>
                                            ...
                                            <div className="relative flex-1 h-full"><div className="absolute inset-0 flex pointer-events-none z-0">{props.visibleDays.map((day, i) => (<div key={i} className={`h-full border-r ${darkMode ? 'border-white/5' : 'border-[#eceff8]'} ${day.isWeekend ? 'bg-black/20' : 'bg-transparent'} ${day.isToday ? 'bg-blue-500/10' : ''} ${!props.showWeekends && i > 0 && day.index > props.visibleDays[i-1].index + 1 ? (darkMode ? 'border-l-2 border-l-[#3e3f4b]' : 'border-l-2 border-l-gray-300') : ''}`} style={{ width: `${zoomLevel}px`, minWidth: `${zoomLevel}px` }}>{day.isToday && <div className="absolute top-0 bottom-0 left-0 w-0.5 bg-blue-500 z-10 opacity-50"></div>}{day.isEndOfWeekToday && <div className="absolute top-0 bottom-0 right-0 w-0.5 bg-blue-500 z-10"></div>}</div>))}</div></div>
                                        </div>
                                        ...
                                        {!isGroupCollapsed && groupTasks.map(task => {
                                            ...
                                            return (
                                                <React.Fragment key={task.id}>
                                                    <div className={`flex border-b relative group/task ${isExpanded ? (darkMode ? 'bg-[#151726]' : 'bg-gray-100') : (darkMode ? 'bg-[#181b34] hover:bg-[#202336]' : 'bg-white hover:bg-[#f9fafc]')} ${darkMode ? 'border-[#2b2c32]' : 'border-[#eceff8]'}`} style={{ height: `${rowHeight}px` }}>
                                                        <div className={`w-80 border-r flex-shrink-0 sticky left-0 z-30 ${darkMode ? 'border-[#2b2c32] bg-inherit text-gray-300' : 'border-[#d0d4e4] bg-inherit text-gray-800'}`} onDragOver={(e) => handleRowDragOver(e, 'task', task.id)} onDrop={handleRowDrop} style={{ opacity: isDragging ? 0.5 : 1, transition: 'opacity 0.2s' }}>
                                                    ...
                                                    {isExpanded && task.subitems.map(sub => {
                                                         ...
                                                         return (
                                                            <div key={sub.id} className={`flex h-10 items-center border-t relative ${darkMode ? 'border-[#2b2c32] hover:bg-[#202336]' : 'border-[#eceff8] hover:bg-[#eceff8]'}`} onDragOver={(e) => handleRowDragOver(e, 'subitem', sub.id)} onDrop={handleRowDrop}>
                                                                    <div className={`w-80 border-r flex-shrink-0 sticky left-0 z-30 ${darkMode ? 'border-[#2b2c32] bg-inherit text-gray-400' : 'border-[#d0d4e4] bg-inherit text-gray-500'}`} onDragOver={(e) => handleRowDragOver(e, 'subitem', sub.id)} onDrop={handleRowDrop} style={{ opacity: isSubDragging ? 0.5 : 1, transition: 'opacity 0.2s' }}>
                                                                        <TaskRow {...subRowProps} />
                                        ...
                                        {!isGroupCollapsed && (<div className={`flex border-b relative ${darkMode ? 'border-[#2b2c32]' : 'border-[#eceff8]'}`} style={{ height: `${rowHeight}px` }}> <div className={`w-80 border-r flex-shrink-0 sticky left-0 z-30 flex items-center px-4 ${darkMode ? 'border-[#2b2c32] bg-[#181b34]' : 'border-[#d0d4e4] bg-white'}`}><div className="w-10 flex justify-center mr-2 opacity-50"><Plus size={14}/></div><input type="text" placeholder="+ Add Item" className={`bg-transparent outline-none text-sm w-full ${darkMode ? 'text-gray-400 placeholder-gray-600' : 'text-gray-500 placeholder-gray-400'}`} onKeyDown={(e) => { if (e.key === 'Enter' && e.target.value.trim()) { addTaskToGroup(project.id, group.id, e.target.value); e.target.value = ''; }}} /></div><div className="relative flex-1 h-full"><div className="absolute inset-0 flex pointer-events-none z-0">{visibleDays.map((day, i) => (<div key={i} className={`h-full border-r ${darkMode ? 'border-white/5' : 'border-[#eceff8]'} ${day.isWeekend ? 'bg-black/20' : 'bg-transparent'} ${!showWeekends && i > 0 && day.index > visibleDays[i-1].index + 1 ? (darkMode ? 'border-l-2 border-l-[#3e3f4b]' : 'border-l-2 border-l-gray-300') : ''}`} style={{ width: `${zoomLevel}px`, minWidth: `${zoomLevel}px` }}></div>))}</div></div></div>)}
                                    </div>
                                 );
                            })}
                        </div>
